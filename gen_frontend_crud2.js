...
type ${Model} = { id: string; ${Model === 'Coupon' ? 'code: string; discountType: string; discountValue: number' : 'title: string; discount: number'}; isActive: boolean };

const columns: ColumnDef<${Model}>[] = [
  { accessorKey: "${Model === 'Coupon' ? 'code' : 'title'}", header: "${Model === 'Coupon' ? 'Code' : 'Title'}" },
  { accessorKey: "${Model === 'Coupon' ? 'discountValue' : 'discount'}", header: "Discount" },
  { 
  ...
  return (
    <div className="max-w-7xl mx-auto p-8 space-y-8">
      ...
      {isLoading ? (
        ...
        <DataTable columns={columns} data={data} searchKey="${Model === 'Coupon' ? 'code' : 'title'}" meta={{ handleEdit, handleDelete }} />
      )}
    </div>
  );
}
