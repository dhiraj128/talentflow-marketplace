...
export default function MessagesPage() {
  ...
  const filteredConversations = conversations.filter(conv => {
    ...
    const name = conv.otherUser?.fullName || conv.otherUser?.email || `Candidate ${otherParticipantId?.substring(0, 4)}`;
              ...
              <div className="p-4 border-b flex items-center gap-3 bg-background">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="font-semibold text-sm md:text-base truncate">
                    {activeConversation.otherUser?.fullName || activeConversation.otherUser?.email || `Candidate ${(activeConversation.participant1Id === user?.id ? activeConversation.participant2Id : activeConversation.participant1Id)?.substring(0, 4)}`}
                  </h4>
