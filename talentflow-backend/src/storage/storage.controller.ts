import { Controller, Get, Param, Req, Res, UseGuards, UnauthorizedException, NotFoundException, ForbiddenException, InternalServerErrorException, BadRequestException } from '@nestjs/common';
  ...
  async serveFile(
    ...
    if (isResume) {
      if (dbUser.role === 'ADMIN') {
        ...
        if (!resumeRecord) throw new ForbiddenException('Not authorized to view this resume');
        ...
        if (!application) throw new ForbiddenException('Not authorized to view this candidate\'s resume');
      } else {
    ...
    try {
      ...
      if (!s3Client || !bucket) {
        throw new InternalServerErrorException('Storage service not configured properly');
      }
