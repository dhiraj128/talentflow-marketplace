  ...
  async createRequest(employerUserId: string, createData: any) {
    ...
    if (!freelancer.isVerified) {
      throw new ForbiddenException(
        'Cannot send request to unverified freelancer',
      );
    }
    ...
    if (existing) {
      throw new ConflictException(
        'You already have an active request with this freelancer',
      );
    }
  ...
  async updateStatus(
    ...
    if (userRole === 'FREELANCER') {
      ...
      if (status !== 'ACCEPTED' && status !== 'REJECTED') {
        throw new ForbiddenException(
          'Freelancers can only ACCEPT or REJECT requests',
        );
      }
      ...
      if (status !== 'COMPLETED') {
        throw new ForbiddenException(
          'Employers can only mark requests as COMPLETED',
        );
      }
      if (request.status !== 'ACCEPTED') {
        throw new ConflictException(
          'Only ACCEPTED active engagements can be marked COMPLETED',
        );
      }
    }
