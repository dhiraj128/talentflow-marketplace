...
export class OffersService {
  ...
  async create(dto: CreateOfferDto, userId: string) {
    ...
    // Check active offer for application
  ...
  async findAllByCandidate(userId: string) {
    ...
    // PRIVACY ENFORCEMENT: Candidate sees non-DRAFT offers
  ...
  async findOne(id: string, userId: string, role: string) {
    ...
    if (role === 'CANDIDATE') {
      ...
      if (offer.status === 'DRAFT')
        throw new ForbiddenException('Draft offers are not visible to candidate');
  ...
  async sendOffer(id: string, userId: string) {
    const offer = await this.findOne(id, userId, 'EMPLOYER');
    if (offer.status !== 'DRAFT') {
      throw new BadRequestException('Offer is already sent or processed');
    }
  ...
  async acceptOffer(id: string, userId: string) {
    ...
    if (!['SENT', 'VIEWED'].includes(offer.status)) {
      throw new BadRequestException(`Offer cannot be accepted in state ${offer.status}`);
    }
    ...
    // Transactional Accept: Offer ACCEPTED & Application HIRED
  ...
  async declineOffer(id: string, declineReason: string | undefined, userId: string) {
    const offer = await this.findOne(id, userId, 'CANDIDATE');

    if (!['SENT', 'VIEWED'].includes(offer.status)) {
      throw new BadRequestException(`Offer cannot be declined in state ${offer.status}`);
    }
  ...
  async withdrawOffer(id: string, userId: string) {
    const offer = await this.findOne(id, userId, 'EMPLOYER');

    if (['ACCEPTED', 'DECLINED', 'EXPIRED'].includes(offer.status)) {
      throw new BadRequestException(`Cannot withdraw offer in terminal status ${offer.status}`);
    }
