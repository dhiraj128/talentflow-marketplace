...
export function ContinueLearningBanner() {
  ...
  const fetchActiveCourse = async () => {
    try {
      ...
      if (list.length > 0) {
        setCourse(list[0].course || list[0]);
      }
    } catch (e) {}
  };
      ...
      <CardContent className="p-6 md:p-8 relative z-10 flex flex-col md:flex-row gap-6 items-center">
        <div className="flex-1 space-y-4 w-full">
          <div>
            ...
            <h2 className="text-2xl md:text-3xl font-bold">{course.title || "Enrolled Course"}</h2>
            <p className="text-blue-100/80 mt-1">{course.category || "General Training"}</p>
          </div>
          ...
          <div className="space-y-1.5 pt-2">
            ...
            <Progress value={course.progress || 0} className="h-2 bg-white/20 [&>div]:bg-white" />
          </div>
        </div>

        <div className="w-full md:w-auto shrink-0 md:pl-6 md:border-l border-white/10 flex flex-col items-center">
          <Link href={`/job-seeker/learning/${course.id}`} className="w-full">
